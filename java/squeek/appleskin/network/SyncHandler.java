...
public class SyncHandler {
	...
	public static void init() {
		ClientSidePacketRegistryImpl.INSTANCE.register(EXHAUSTION_SYNC, (packetContext, packetByteBuf) -> {
			float exhaustion = packetByteBuf.readFloat();
			MinecraftClient.getInstance().execute(() -> HungerHelper.setExhaustion(packetContext.getPlayer(), exhaustion));
		});
		ClientSidePacketRegistryImpl.INSTANCE.register(SATURATION_SYNC, (packetContext, packetByteBuf) -> {
			float saturation = packetByteBuf.readFloat();
			MinecraftClient.getInstance().execute(() -> packetContext.getPlayer().getHungerManager().setSaturationLevel(saturation));
		});
	}
