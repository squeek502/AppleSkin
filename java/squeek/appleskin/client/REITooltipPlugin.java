...
public class REITooltipPlugin // implements REIClientPlugin
{
	...
	// converted into a TooltipComponent. That means, AppleSkin's custom
	// TooltipComponent is bypassed and our custom rendering doesn't happen.

	// We need to do the conversion before that, so we register a
	// tooltip transformer with REI,

	// find our FoodOverlayTextComponent and convert it into AppleSkin's custom
	// TooltipData implementation and add *that* directly to REI's tooltip,
	// so that when it gets converted to a TooltipComponent later.
	// We can handle it ourselves (see our TooltipComponentMixin).
